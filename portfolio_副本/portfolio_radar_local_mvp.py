...
@media(max-width:900px){.workspace,.metrics,.grid,.row{grid-template-columns:1fr}.nav{align-items:flex-start;gap:8px;flex-direction:column}.main{padding:16px 14px}.fund-head{flex-direction:column}}
</style>
</head>
<body>
  ...
  <div class="workspace">
    <div class="left-col">
      <div class="grid">
        ...
        <div class="panel">
          ...
          <div class="actions">
            <button onclick="fetchHoldings()">尝试自动抓取持仓</button>
            <button class="primary" onclick="uploadHoldings()">保存手动持仓</button>
          </div>
    ...
    <div class="right-col">
      <div class="panel">
        ...
        <label class="hint" style="display:flex;gap:8px;align-items:center;background:#fff"><input id="keepKey" type="checkbox" style="width:auto">仅本页面暂存到今日 15:00，用于 12:00 / 14:40 盘中刷新</label>
        <div class="seg">
        ...
        <div class="actions">
          <button class="primary" onclick="analyzeActiveFund()">AI 分析当前基金新闻</button>
          <button onclick="watchActiveFund()">AI 看盘</button>
          <button onclick="runFollowup()">事后回访</button>
          <button onclick="backfillWeek()">回填近一周数据</button>
        </div>
...
function afterApiUse(rawKey){
  if($("keepKey").checked && rawKey){
    const expires = todayAt1500();
    if(Date.now() < expires){
      sessionApiKey = rawKey;
      sessionApiKeyExpiresAt = expires;
      $("aiKey").value = "";
...
function renderFund(f){
  ...
  return `<div class="fund">
    ...
    <div class="bar-row"><span class="bar-label">技术</span><div class="bar"><div class="fill" style="width:${pct(s.conf_tech)}"></div></div><span>${pct(s.conf_tech)}</span></div>
    <div class="bar-row"><span class="bar-label">事件</span><div class="bar"><div class="fill" style="width:${pct(s.conf_event)}"></div></div><span>${pct(s.conf_event)}</span></div>
    <div class="bar-row"><span class="bar-label">情绪</span><div class="bar"><div class="fill" style="width:${pct(s.conf_senti)}"></div></div><span>${pct(s.conf_senti)}</span></div>
    <div class="hint ${hs.length ? "ok" : "warn"}">${s.note || "已计算。"}</div>
    ${hs.length ? `<table class="table"><tr><th>股票代码</th><th>权重</th></tr>${hs.slice(0,10).map(h=>`<tr><td>${h.stock_code}</td><td>${pct(h.weight)}</td></tr>`).join("")}</table>` : ""}
...
async function showHistory(fund_code){
  ...
  try{
    ...
    showModal("历史数据回顾", `${summary}<table class="table"><tr><th>时间</th><th>综合</th><th>建议</th><th>持仓涨跌</th><th>结果</th></tr>${rows || `<tr><td colspan="5">暂无历史记录</td></tr>`}</table>`);
  }finally{ clearBusy(); }
}
refresh();
    ...
    with connect() as conn:
        ...
        for row in conn.execute("SELECT * FROM holding ORDER BY fund_code, weight DESC"):
            holdings.setdefault(row["fund_code"], []).append(
                {"stock_code": row["stock_code"], "weight": row["weight"]}
            )
        signals = {
            row["fund_code"]: {
...
def collect_news(group: str, limit: int = 12) -> list[dict]:
    ...
    try:
        ...
        for fetcher_name, fetcher, title_keys, url_keys, source_keys, time_keys in [
            (
                "东方财富",
...
def fetch_stock_watch(stock_code: str) -> dict:
    ...
    if item["change_1d"] is None and cached_rows:
        ...
        item.update({key: latest.get(key) for key in ["change_1d", "change_5d", "volume_ratio", "super_net_in", "money_flow_score", "tech_score"] if latest.get(key) is not None})
...
def build_market_watch_prompt(fund: dict, holdings: list[dict], watch_rows: list[dict], preference: str) -> str:
    need_steps = preference == "detailed"
    rows_text = "\n".join(
        f"- {row['stock_code']}，权重 {round(row.get('weight', 0) * 100, 2)}%，领域 {row.get('sector')}，1日涨跌 {row.get('change_1d')}，5日涨跌 {row.get('change_5d')}，量比 {row.get('volume_ratio')}，主力/超大单净流入 {row.get('super_net_in')}，本地技术分 {row.get('tech_score')}"
...
def intraday_refresh_once(slot: str) -> None:
    ...
    for fund in current.get("funds", []):
        ...
        if backfill.get("error") or signal.get("error"):
            ...
            body = f"已刷新近一周行情/资金流，新增 {backfill.get('history_rows', 0)} 条启动样本。"
